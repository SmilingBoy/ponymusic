 ...
 */
@AndroidEntryPoint
class SearchArtistFragment : SimpleMusicRefreshFragment<NmArtistData>() {
    private val viewModel by activityViewModels<SearchViewModel>()
    private val itemBinder by lazy {
        SearchArtistItemBinder(object : OnItemClickListener2<NmArtistData> {
            override fun onItemClick(item: NmArtistData, position: Int) {
//                playerController.addAndPlay(item.toNmMediaItem())
//                CRouter.with(context).url(RoutePath.PLAYING).start()
            }

            override fun onMoreClick(item: NmArtistData, position: Int) {
...
//                    .show()
    ...
    override fun initAdapter(adapter: RAdapter<NmArtistData>) {
        adapter.register(itemBinder)
    }

    override suspend fun getData(page: Int): CommonResult<List<NmArtistData>> {
        ...
        val res = runCatching {
            DiscoverApi.Companion.get().getArtistList(
